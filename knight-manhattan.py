        ...
        for row in self.board:
            for cell in row:
                if not cell.visited:
                    return False
        if tour:
            print(f"I deem that from this position of {self.x, self.y} I can return to the starting position")
            # check if the knight can move to the starting position
            if self.get_cell(self.x, self.y).siren(Knight()):
                return True
            else:
                return False
        return True
